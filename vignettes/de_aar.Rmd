...
This vignette demonstrates the use of the `pvwattsOlmoSkoplaki` package for modeling the Mulilo De Aar PV solar plant in South Africa. The plant specifications are publicly available from the [HAWI Knowledge Database](https://hawiknowledge.org/solar_power_stations_2.html#DeAarMulilo).
...
### Transposition Model: Olmo

We use the Olmo transposition model to convert global horizontal irradiance (GHI) to plane-of-array (POA) irradiance. This model accounts for:

- Direct beam irradiance component (using sun position and panel orientation)
- Diffuse sky irradiance (isotropic assumption)
- Ground-reflected irradiance (albedo = 0.2, typical for semi-arid regions)

The Olmo model is computationally efficient and suitable for the South African climate with high direct normal irradiance.
...
- Ayvazoğluyüksel, Ö., & Başaran Filik, Ü. (2018). Estimation methods of global solar radiation, cell temperature and solar power forecasting: A review and case study in Eskişehir. *Renewable and Sustainable Energy Reviews*, 91, 639-653.
